...
public interface FornecedorLicitacaoRepository extends JpaRepository<FornecedorLicitacao,Long>{
    ...
    @Query("select distinct f from FornecedorLicitacao f left join f.itens i where :q='' or lower(f.nome) like lower(concat('%',:q,'%')) or lower(f.resumo) like lower(concat('%',:q,'%')) or lower(i.nome) like lower(concat('%',:q,'%')) or lower(i.marca) like lower(concat('%',:q,'%')) order by f.nome")
