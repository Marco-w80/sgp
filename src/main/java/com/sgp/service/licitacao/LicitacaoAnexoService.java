...
public class LicitacaoAnexoService {
    ...
    @Transactional public LicitacaoAnexo salvar(Long licitacaoId,Long itemId,EtapaLicitacao etapa,MultipartFile arquivo)throws IOException{
        ...
        LicitacaoAnexo a=new LicitacaoAnexo();a.setLicitacao(l);a.setItem(item);a.setEtapa(etapa);a.setNomeOriginal(seguro);a.setNomeArmazenado(armazenado);a.setCaminho(raiz.relativize(destino).toString());a.setMimeType(Optional.ofNullable(arquivo.getContentType()).orElse("application/octet-stream"));a.setTamanho(arquivo.getSize());a.setEnviadoPor(usuarioAtual.obter());
