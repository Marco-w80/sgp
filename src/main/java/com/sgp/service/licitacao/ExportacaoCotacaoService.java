...
public class ExportacaoCotacaoService {
    ...
    private void linha(StringBuilder s,LicitacaoItem i,CotacaoLicitacao c){campo(s,i.getNumeroItem());campo(s,i.getDescricao());campo(s,numero(i.getQuantidade()));campo(s,i.getUnidade());campo(s,numero(i.getValorReferencia()));campo(s,c==null?null:c.getFornecedorNome());campo(s,c==null?null:numero(c.getValorUnitario()));campo(s,c==null||c.getDataCotacao()==null?null:c.getDataCotacao().format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));ultimo(s,c==null?null:c.getObservacoes());}
    private String numero(BigDecimal valor){return valor==null?null:valor.stripTrailingZeros().toPlainString().replace('.',',');}
