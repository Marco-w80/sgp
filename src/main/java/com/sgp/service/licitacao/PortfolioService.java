...
public class PortfolioService {
    ...
    @Transactional public FornecedorLicitacao salvarFornecedor(Long id,String nome,String contato,String resumo,String observacoes){if(nome==null||nome.isBlank())throw new IllegalArgumentException("Nome é obrigatório.");FornecedorLicitacao f=id==null?new FornecedorLicitacao():fornecedores.findById(id).orElseThrow();f.setNome(nome);f.setContato(contato);f.setResumo(resumo);f.setObservacoes(observacoes);if(id==null)f.setCriadoPor(usuarioAtual.obter());return fornecedores.save(f);}
