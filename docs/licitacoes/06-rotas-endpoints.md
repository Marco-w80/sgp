...
O índice tem duas tentativas curtas de até 6 segundos. As APIs detalhada e de itens têm uma tentativa de até 40 segundos por chamada. A consulta de itens usa páginas de 50 registros e continua até receber uma página incompleta; se uma página falhar, nenhuma lista parcial é aplicada. Respostas completas ficam em cache de memória por 5 minutos. As chamadas usam HTTP/1.1 explícito, `User-Agent` da aplicação e redirecionamentos desabilitados.
